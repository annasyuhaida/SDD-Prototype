...
class Request_Marriage_ctrl {
    
    public function ApplyMarriageRequest($Lid,$appID) {
        ...
        if($Lid == 2){
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                require_once '../App/View/ManageMarriageRequest/RequestMarriageForm.php' ;
                $view = new RequestMarriageForm();
                $view->render($apps,$partner,$Lid,$appID);
        ...
        if($Lid == 3){
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                //echo "<script>console.log('Debug Objects: " . $applicantPhoneNo . "' );</script>";
                // Perform actions to create a user in the database
                $model->createNewMarriageRequest($appID, $rm_Country,$rm_Date, $rm_Mahr_Amount, $rm_Address, $rm_Mahr_Type, $rm_Witness_name, $rm_Witness_IC, $rm_Witness_Address, $rm_Witness_Phone_No, $rm_Marriage_Category);
    ...
    public function EditMarriageRequest($Lid,$appID,$reqID) {
        ...
        if($Lid == 0){
            ...
            require_once '../App/View/ManageMarriageRequest/EditRequestMarriageForm.php' ;
            $view = new EditRequestMarriageForm();
    ...
    public function m($appID) {
        $model = new MarriageRequestModel();

        $request = $model->printAppPartReqID($appID);

        require_once '../plugins/fpdf/generate_pdf2.php';
        $view = new Printpdf();
        $view->render($request);
    }
    ...
    public function MarriageRequestList($appID) {
        ...
        $marr = $model->updateAppPartReqID($appID);
        
        require_once '../App/View/ManageMarriageRequest/RequestMarriageList.php' ;
        $view = new RequestMarriageList();
        $view->render($marr,$appID);
    }
